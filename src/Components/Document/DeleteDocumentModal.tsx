...
const DeleteDocumentModal: React.FC<DeleteDocumentModalProps> = ({
  ...
  function getFileIcon(type: "pdf" | "img") {
    if (type === "pdf") {
      return (
        <svg
          className="size-7"
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm p-4">
      <div className="bg-[#FFF7E8] rounded-2xl px-6 sm:px-8 md:px-10 py-6 sm:py-8 min-w-lg shadow-2xl relative flex flex-col items-start border border-[var(--color-modal-border)] max-w-md w-full">
        <button
        ...
        <p className="text-3xl font-lighter flex items-center gap-3 font-serif mb-2 mt-2">
          Remove document?
        </p>
        <div className="w-full text-left mb-6 sm:mb-8 mt-4">
          <p className="block text-[var(--color-modal-foreground)] mb-2 font-medium text-sm sm:text-base">
            Are you sure you want to delete
          </p>
          <div className="flex items-center gap-3 bg-[#EBD5BD] rounded-xl px-4 py-3 mb-2">
            {getFileIcon(fileType)}
            <span className="font-semibold text-left text-[var(--color-modal-foreground)] text-base truncate max-w-[180px]">
              {documentName}
            </span>
          </div>
        </div>
